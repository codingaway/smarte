...
--
-- Host: 127.0.0.1
-- Generation Time: Aug 11, 2016 at 04:04 PM
...
-- Procedures
--
CREATE DEFINER=`smarte`@`%` PROCEDURE `update_zone` (IN `zone_code` INT, IN `security_id` INT, IN `noise` DOUBLE, IN `temp` DOUBLE, IN `light` DOUBLE, IN `guest_id` VARCHAR(16))  MODIFIES SQL DATA
BEGIN
INSERT INTO zone_data VALUES(zone_code, security_id, noise, light, temp, guest_id, now());
END$$
...
INSERT INTO `zone_data` (`zone_code`, `security_id`, `noise_level`, `light_level`, `temp`, `guest_id`, `datetime`) VALUES
(746, 234, 23, 341234, 324, '\'akkvv\'', '2016-08-11 17:03:48');
